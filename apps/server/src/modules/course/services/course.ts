...
export default class CourseService {
  ...
  public static async getAllCourses(
    ...
    return await CourseModel.getAllCourses(page, limit, isBase);
  }

  public static async getAllCourseGroups(page: number, limit: number) {
    return await CourseGroupModel.getAllCourseGroups(page, limit);
  }

  public static async createBaseCourse(
    course: DTOBodyType<typeof CourseDTO.createBaseCourse>,
  ) {
    await CourseModel.createBaseCourse(course);
  }

  public static async createCourse(
    course: DTOBodyType<typeof CourseDTO.createCourse>,
  ) {
    await CourseModel.createCourse(course);
  }

  public static async createCourseGroup(
    courseGroup: DTOBodyType<typeof CourseDTO.createCourseGroup>,
  ) {
    await CourseGroupModel.createCourseGroup(courseGroup);
  }

  public static async deleteCourseGroup(
    courseGroup: DTOBodyType<typeof CourseDTO.deleteCourseGroup>,
  ) {
    if (courseGroup.id)
      await CourseGroupModel.deleteCourseGroup({ groupId: courseGroup.id });
    if (courseGroup.name)
      await CourseGroupModel.deleteCourseGroup({ groupName: courseGroup.name });
  }

  public static async deleteCourseGroups(
    courseGroup: DTOBodyType<typeof CourseDTO.deleteCourseGroups>,
  ) {
    if (courseGroup.ids)
      await CourseGroupModel.deleteCourseGroups({ groupIds: courseGroup.ids });
    if (courseGroup.names)
      await CourseGroupModel.deleteCourseGroups({
        groupNames: courseGroup.names,
      });
  }
  ...
  public static async enrollUsersInCourse({
    usernames,
    courseIds,
  }: DTOBodyType<typeof CourseDTO.enrollUsersInCourse>) {
    ...
    users.forEach((user) => {
      ...
      const coursesToBeEnrolledIn = courses.filter(
  ...
  public static async addTeachersInCourse({
    usernames,
    courseIds,
  }: DTOBodyType<typeof CourseDTO.addTeachersInCourse>) {
    let users = await UserModel.getUsersByUsernames(usernames);
    // TODO: replace with includes once multiple user types are implemented
  ...
  public static async editCourse(
    ...
    if (courseEdits.name) {
      ...
      if (childCourses && childCourses.length)
        childCourses.forEach((childCourse) => {
          if (courseEdits.name) childCourse.name = courseEdits.name;
        });
    ...
    if (courseEdits.code) {
      ...
      if (childCourses && childCourses.length)
        childCourses.forEach((childCourse) => {
          if (courseEdits.code) childCourse.code = courseEdits.code;
        });
      await CourseModel.saveCourses(childCourses);
    }
    if (courseEdits.image) {
      ...
      if (childCourses && childCourses.length)
        childCourses.forEach((childCourse) => {
          if (courseEdits.image) childCourse.image = courseEdits.image;
        });
      await CourseModel.saveCourses(childCourses);
    }
    ...
    if (courseEdits.academicDuration)
      course.academicDuration = courseEdits.academicDuration;
    if (courseEdits.academicYear)
      course.academicYear = courseEdits.academicYear;
    await CourseModel.saveCourse(course);
  }

  public static async editCourseGroup(
    courseGroupEdits: DTOBodyType<typeof CourseDTO.editCourseGroup>,
  ) {
    const courseGroup = await CourseGroupModel.getCourseGroup({
      groupId: courseGroupEdits.id,
    });
    ...
    if (
      courseGroupEdits.courseIdsToAdd &&
      courseGroupEdits.courseIdsToAdd.length
    ) {
      const coursesToAdd = await CourseModel.getCourses(
        courseGroupEdits.courseIdsToAdd,
      );
      groupCourseCourses.push(...coursesToAdd);
    }
    if (
      courseGroupEdits.courseIdsToRemove &&
      courseGroupEdits.courseIdsToRemove.length
    ) {
      courseGroup.courses = Promise.resolve(
        groupCourseCourses.filter(
          (course) => !courseGroupEdits.courseIdsToRemove?.includes(course.id),
        ),
      );
    }
  ...
  public static async editCoursePrerequisiteCourses(
    courseEdits: DTOBodyType<typeof CourseDTO.editCoursePrerequisiteCourses>,
  ) {
    const course = await CourseModel.getCourse(courseEdits.id);
    if (!course) throw new API_ERROR(API_MESSAGES.DOESNT_EXIST);
    ...
    if (!course) throw new API_ERROR(API_MESSAGES.DOESNT_EXIST);
    if (
      courseEdits.prerequisiteCourseIdsToAdd &&
      courseEdits.prerequisiteCourseIdsToAdd.length
    ) {
      const prerequisiteCoursesToAdd = await CourseModel.getCourses(
        courseEdits.prerequisiteCourseIdsToAdd,
      );
    ...
    if (
      courseEdits.prerequisiteCourseIdsToRemove &&
      courseEdits.prerequisiteCourseIdsToRemove.length
    ) {
      prerequisiteCourses = prerequisiteCourses.filter(
        (course) =>
          !courseEdits.prerequisiteCourseIdsToRemove?.includes(course.id),
      );
      course.prerequisiteCourses = Promise.resolve(prerequisiteCourses);
    }
    await CourseModel.saveCourse(course);
  ...
  public static async editCoursePrerequisiteCourseGroups(
    courseEdits: DTOBodyType<
      typeof CourseDTO.editCoursePrerequisiteCourseGroups
    >,
  ) {
    const course = await CourseModel.getCourse(courseEdits.id);
    if (!course) throw new API_ERROR(API_MESSAGES.DOESNT_EXIST);
    if (
      courseEdits.prerequisiteCourseGroupIdsToAdd &&
      courseEdits.prerequisiteCourseGroupIdsToAdd.length
    ) {
      const prerequisiteCourseGroupsToAdd =
        await CourseGroupModel.getCourseGroups(
          courseEdits.prerequisiteCourseGroupIdsToAdd,
        );
      course.prerequisiteCourseGroups.push(...prerequisiteCourseGroupsToAdd);
    }
    if (
      courseEdits.prerequisiteCourseGroupIdsToRemove &&
      courseEdits.prerequisiteCourseGroupIdsToRemove.length
    ) {
      course.prerequisiteCourseGroups = course.prerequisiteCourseGroups.filter(
        (course) =>
          !courseEdits.prerequisiteCourseGroupIdsToRemove?.includes(course.id),
      );
    }
    await CourseModel.saveCourse(course);
  }
  ...
  public static async searchCourseByName(name: string, isBase: boolean) {
    return await CourseModel.searchCourseByName(name, isBase);
  }

  public static async searchCourseGroupByName(name: string) {
    return await CourseGroupModel.searchCourseGroupByName(name);
  }
