...
export default class CourseController {
  ...
  public static async getStudentCourses(
    ...
    switch (option) {
      ...
      case studentCoursesOptions.COMPLETED:
        const completedCourses = await CourseService.getUserCompletedCourses(
          user.username,
        );
        completedCourses.forEach(
          (course) => ((course as any).completed = true),
        );
        return res.status(StatusCodes.OK).json({ courses: completedCourses });
      case studentCoursesOptions.ALL:
        const { enrolled, completed } = await CourseService.getAllUserCourses(
          user.username,
        );
        enrolled.forEach((course) => ((course as any).enrolled = true));
        completed.forEach((course) => ((course as any).completed = true));
        return res.status(StatusCodes.OK).json({ enrolled, completed });
      default:
        return res
  ...
  public static async getUserTeachingCourses(req: Request, res: Response) {
    const user = getUserFromRequest(req);
