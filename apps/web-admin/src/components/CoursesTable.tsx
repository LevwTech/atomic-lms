...
export default function CoursesTable({ headers, data, refetch }: TableProps) {
  ...
  const deleteCourse = async () => {
    try {
      setLoading(true);
      const response = await fetch(`http://localhost:3000/course/course`, {
        method: "DELETE",
        headers: {
        ...
        {data.map((item) => (
          <div className="flex w-full items-center py-5 border-b text-sm">
            ...
            <span className="w-full h-fit pl-4 flex items-center gap-4 text-sm text-blue-400">
              <Link to={`/courses/edit/${item.id}`} className="cursor-pointer">
                Edit
              </Link>{" "}
              <p
                onClick={() => {
